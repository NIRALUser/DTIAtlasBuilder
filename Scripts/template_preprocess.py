...
if config['m_useGridProcess']:
  ...
  import time 
  FilesFolder= config['m_OutputPath'] + '/DTIAtlas/GridProcessingFiles'
...
if config['m_RegType']==0:
  ...
  if config['m_useGridProcess']:
    RescaleTempCommand= "" + config['m_GridGeneralCommand'] + " " + config['m_PythonPath'] + " " + config['m_OutputPath'] + "/DTIAtlas/Script/RunCommandOnServer.py " + FilesFolder + "/file \\'" + RescaleTempCommand  + "\\'"
    ...
    if not CheckFileExists(ScalarMeasurement, 0, config["m_CasesIDs"][0] ) :
      ...
      if config['m_useGridProcess']:
        if CropDTICase1Template or GeneScalarMeasurementCase1Template :
          GridCase1TemplateCommand= "" + config['m_GridGeneralCommand'] + " " + config['m_PythonPath'] + " " + config['m_OutputPath'] + "/DTIAtlas/Script/RunCommandOnServer.py " + FilesFolder + "/file"
...
# Generating FA/MA of registered images
    ...
    if config["m_useGridProcess"]:
      if len(GridProcessCaseCommandsArray)!=0 : # There are operations to run
        GridAffineCommand= config["m_GridGeneralCommand"] + " " + config["m_PythonPath"] + " " + config["m_OutputPath"] + "/DTIAtlas/Script/RunCommandOnServer.py " + FilesFolder + "/Case" + str(case+1)
        GridCmd = 0
  ...
  if config["m_nbLoops"]!=0:
    if n != int(config["m_nbLoops"]) : # this will not be done for the last lap
      ...
      while case < len(allcases):
        ...
        case += 1
      if config["m_useGridProcess"]:
        AverageCommand= config["m_GridGeneralCommand"] + " " + config["m_PythonPath"] + " " + config["m_OutputPath"] + "/DTIAtlas/Script/RunCommandOnServer.py " + FilesFolder + "/file '" + AverageCommand  + "'"
